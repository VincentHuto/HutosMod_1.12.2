...
public class ModelBlueTuningFork extends ModelBase {
	// fields
	ModelRenderer Stem;
	...
	public void renderForks(int cubes, int repeat,int origRepeat) {
		...
		for (int i = 0; i < cubes; i++) {
			...
			if (repeat < origRepeat) {
				GlStateManager.color(.1F, 0.2F, 3.0F, (float) repeat / (float) origRepeat * 0.4F);
				GlStateManager.enableBlend();
				GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
				GlStateManager.disableAlpha();
			} else
				GlStateManager.color(0.1F, 0.3F, 0.3F, 0F);
