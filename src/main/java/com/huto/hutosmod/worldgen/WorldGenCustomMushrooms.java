...
public class WorldGenCustomMushrooms extends WorldGenerator {

	public static Block mushroomType;

	public WorldGenCustomMushrooms(Block mush) {
		this.mushroomType = mush;
	}
	...
	public boolean generate(World worldIn, Random rand, BlockPos pos) {
		...
		// debug:
		// System.out.println("Y-value of ground is " + y + " at (" + pos.getX() + ", "
		// + pos.getZ() + ")");
		...
		if (y >= pos.getY()) {
			...
			if (toReplace == Blocks.AIR || toReplace.getMaterial(toReplace.getDefaultState()) == Material.PLANTS) {
				...
				// use 2 as the flag to prevent update -- you don't have to include that
				// parameter
				worldIn.setBlockState(bushPos, mushroomType.getDefaultState(), 2);
				// debug:
				// System.out.println("placed a cookie bush!");
			} // else System.out.println("Sadly, this block is occupied by " +
	...
	public static int getGroundFromAbove(World world, int x, int z) {
		...
		while (!foundGround && y-- >= 0) {
			...
			foundGround = blockAt == Blocks.STONE || blockAt == Blocks.MYCELIUM
