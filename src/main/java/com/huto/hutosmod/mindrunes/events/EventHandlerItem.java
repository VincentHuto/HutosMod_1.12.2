...
{
	private static ResourceLocation capabilityResourceLocation = new ResourceLocation(Reference.MODID, "bauble_cap");
	...
	*/
	@SubscribeEvent(priority = EventPriority.LOWEST)
 	public void itemCapabilityAttach(AttachCapabilitiesEvent<ItemStack> event)
 	{
		...
		if (stack.isEmpty() || !(stack.getItem() instanceof IRune) || stack.hasCapability(RuneCapabilities.CAPABILITY_ITEM_RUNE, null)
				|| event.getCapabilities().values().stream().anyMatch(c -> c.hasCapability(RuneCapabilities.CAPABILITY_ITEM_RUNE, null)))
			return;
		...
		event.addCapability(capabilityResourceLocation, new ICapabilityProvider() {
			...
			public boolean hasCapability(@Nonnull Capability<?> capability, @Nullable EnumFacing facing) {
				return capability == RuneCapabilities.CAPABILITY_ITEM_RUNE;
			}
			...
			public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
				return capability ==RuneCapabilities.CAPABILITY_ITEM_RUNE
						? RuneCapabilities.CAPABILITY_ITEM_RUNE.cast((IRune) stack.getItem())
						: null;
			}
		});
	}
