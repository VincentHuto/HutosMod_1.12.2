...
public class ManaHopperCodeHooks
{
    /**
     * Copied from TileEntityManaHopper#captureDroppedItems and added capability support
    ...
    /**
     * Copied from TileEntityManaHopper#transferItemsOut and added capability support
     */
    public static boolean insertHook(TileEntityManaHopper hopper)
    {
    ...
    /**
     * Copied from TileEntityManaHopper#insertStack and added capability support
     */
    private static ItemStack insertStack(TileEntity source, Object destination, IItemHandler destInventory, ItemStack stack, int slot)
    {
        ...
        if (destInventory.insertItem(slot, stack, true).isEmpty())
        {
            ...
            if (insertedItem)
            {
                if (inventoryWasEmpty && destination instanceof TileEntityManaHopper)
                {
                    TileEntityManaHopper destinationHopper = (TileEntityManaHopper)destination;

                    if (!destinationHopper.mayTransfer())
                    {
                        int k = 0;

                        if (source instanceof TileEntityManaHopper)
                        {
                            if (destinationHopper.getLastUpdateTime() >= ((TileEntityManaHopper) source).getLastUpdateTime())
                            {
                                k = 1;
                            }
                        }
