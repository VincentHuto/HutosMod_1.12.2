...
public class GuiTomePage extends GuiScreen {
	...
	int guiHeight = 228;
	int left, top;
	final int BUTTONCLOSE = 0, ARROWF = 1, ARROWB = 2;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (pageNum != (RegistryHandler.pageList.size() - 1)) {
	...
	public void initGui() {
		...
		textBox = new GuiTextField(0, fontRenderer, left - guiWidth + 150, top + guiHeight - 227, 20, 20);
		updateButtons();
		super.initGui();
	...
	public void updateTextBoxes() {
		...
		if (!textBox.getText().isEmpty()) {
			if (!textBox.isFocused()) {
				...
				if (searchNum < RegistryHandler.pageList.size()) {
					mc.displayGuiScreen(RegistryHandler.pageList.get(searchNum));
				} else if (searchNum >= RegistryHandler.pageList.size()) {
					mc.displayGuiScreen(RegistryHandler.pageList.get(RegistryHandler.pageList.size() - 1));
				}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		case ARROWF:
			if (pageNum != (RegistryHandler.pageList.size() - 1)) {
				mc.displayGuiScreen(RegistryHandler.pageList.get((pageNum + 1)));
				break;

			} else if (pageNum == (RegistryHandler.pageList.size() - 1)) {
				mc.displayGuiScreen(RegistryHandler.pageList.get((pageNum)));
				break;
			}
