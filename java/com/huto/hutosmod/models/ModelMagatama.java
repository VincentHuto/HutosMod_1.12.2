 ...
 */
public class ModelMagatama extends ModelBase {
    ...
    public void renderMagatamas(int cubes, int repeat, int origRepeat) {
		...
		for(int i = 0; i < cubes; i++) {
			...
			if(repeat < origRepeat) {
				GlStateManager.color(.2F, 0F, 0.2F, (float) repeat / (float) origRepeat * 0.4F);
				GlStateManager.enableBlend();
				GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
				GlStateManager.disableAlpha();
			} else GlStateManager.color(0.0F, 0F, 0.0F, 0F);
